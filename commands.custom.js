...
module.exports = function(ChunkBot) {
	...
	ChunkBot.addAdmin("Goat Party");

	// Setup message that displays on start up.
	ChunkBot.config.botIdent = "/me :eggplant:";

	// Output song statistics after each play?
	ChunkBot.config.outputSongStats = false;
	...
	*/
	...
	ChunkBot.addCommand({ // Test command.
			...
			text: /.*\bbot\b.*\b(dance|boog(y|ie)|twerk|fire|get funky|crank dat|emoji-1f525)\b.*/ig,
			title: "bot dance",
			...
			text: /.*\bbot\b.*\b(lame|poop|emoji-1f4a9|meh)\b.*/ig,
			title: "bot lame",
			callback: function(data, matches) {
			...
			text: /bot stop being gay?/i,
			callback: function(data) {
				ChunkBot.say("Only one more dick! @" + data.from);
			}
		})
		.addCommand({
			text: /bot fortune?/i,
			hide: true,
			callback: function(data) {
				ChunkBot.sayRandom(["Your fortune: Reply hazy, try again",
		...
		.addCommand({
			...
			hide: true,
			callback: function(data) {
				ChunkBot.sayRandom(["BANG :skull::gun:",
					"/me click",
					"/me click",
					"/me click",
					"/me click",
			...
			text: /(aziz|bot) light/i,
			callback: function(data) {
				ChunkBot.say("/me :sunny:██  :trophy:");
			}
		}).addCommand({
			text: /bot roll?/i,
			hide: true,
			callback: function(data) {
				...
				ChunkBot.say("@" + data.from + " You rolled a " + randomNum);
				var last2 = randomNum.substr(member.length - 2);
				var last3 = randomNum.substr(member.length - 2);
				var dubsA = [11, 22, 33, 44, 55, 66, 77, 88, 99, 00];
				var tripsA = [000, 111, 222, 333, 444, 555, 666, 777, 888, 999];
				var dubsYN = 0;
				var tripsYN = 0;

				for (var i = 0; i < dubsA.length; i++) {
					if (dubsA[i] === last2) {
						dubsYN = 1;
					}
				};
				...
				if (tripsYN == 1 && dubsYN == 1) {
					ChunkBot.say("/me HOLY SHIT. LE EBIN TRIPS");
				} else if (dubsYN == 1) {
					ChunkBot.say("/me le ebin dubs");
				} else {
					ChunkBot.say("lol faggot");
				};
			...
			text: /epic/i,
			callback: function(data) {
				ChunkBot.say("@" + data.from + " don't you mean ebin?");
			}
		}).addCommand({ // Test command.
			text: /^\s*bot( unicorn)?\s*$/,
			hide: true,
			callback: function(data) {
				// Only respond to admins.
				if (!ChunkBot.isAdmin(data.from)) return;
	...
	ChunkBot.addCommand({ // List all configured commands.
		...
		callback: function(data) {
			...
			ChunkBot.say("Goat Party disabled this command since I'm a worthless broken robot. Instead, he makes me say this now.");
			return;
	...
	ChunkBot.addCommand({
		...
		callback: function(data) {
			if (!ChunkBot.eggs()) return;
			ChunkBot.say("Merry Christmas Lad");
		}
	});
